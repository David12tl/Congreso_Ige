  ...
  if (loading || !global) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-light text-xs uppercase tracking-widest">Calculando métricas en DB...</p>
      </div>
    )
  }
    ...
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto p-4 md:p-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GlassCard className="p-6" glowColor="blue">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-4 font-light">Exporta el listado completo de asistentes, tickets y métricas del evento.</p>
        ...
        <GlassCard className="p-6" glowColor="purple">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-4 font-light">Estadísticas detalladas de cada Unidad Académica y su rendimiento.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <GlassCard className="p-6 lg:col-span-2" glowColor="emerald">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold uppercase tracking-wider text-[#0f172a] dark:text-white">Tickets por Unidad Académica (DB)</h3>
            </div>
          ...
          <div className="space-y-5">
            {metricasUA.length === 0 ? (
              <p className="text-xs text-slate-500 dark:text-slate-400 font-light py-8 text-center">No hay registros de tickets comprados en la base de datos.</p>
            ) : (
              metricasUA.map((item, index) => (
                <div key={item.nombre} className="space-y-1.5">
                  <div className="flex justify-between text-xs font-light">
                    <span className="text-slate-700 dark:text-slate-200 font-light">{item.nombre}</span>
                    <span className="text-slate-500 dark:text-slate-400">{item.totalTickets} {item.totalTickets === 1 ? 'ticket' : 'tickets'} ({item.porcentaje}%)</span>
                  </div>
                  <div className="w-full h-3 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full">
                    <div 
        ...
        <GlassCard className="p-6" glowColor="cyan">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-sm font-bold uppercase tracking-wider text-[#0f172a] dark:text-white">Segmentación por Tipo</h3>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col items-center justify-center py-2">
              <div className="w-28 h-28 rounded-full border-4 border-dashed border-cyan-200 flex flex-col items-center justify-center bg-cyan-50">
                <span className="text-2xl font-black text-[#0f172a] dark:text-white">{global.totalTickets}</span>
                <span className="text-[9px] font-light text-slate-500 dark:text-slate-400 uppercase tracking-widest">Total Global</span>
              </div>
            </div>

            {/* Barra Combinada según los tipos del constraint check (alumno / empresa) */}
            <div className="space-y-4">
              <div className="w-full h-4 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md flex overflow-hidden">
              ...
              <div className="space-y-2 text-xs font-light pt-2">
                <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-700 dark:text-slate-200 font-light">Alumnos</span>
                  </div>
                  <span className="text-[#0f172a] dark:text-white font-bold">{global.totalAlumnos} ({global.porcentajeAlumnos}%)</span>
                </div>

                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <HiOutlineBriefcase className="text-purple-700 w-4 h-4" />
                    <span className="text-slate-700 dark:text-slate-200 font-light">Empresas</span>
                  </div>
                  <span className="text-[#0f172a] dark:text-white font-bold">{global.totalEmpresas} ({global.porcentajeEmpresas}%)</span>
                </div>
              </div>
            </div>
          </div>
