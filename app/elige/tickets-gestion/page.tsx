...
import { obtenerTicketsPorRol, AsistenteTicket } from './actions'
...
export default function TicketsGestionPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const response = await obtenerTicketsPorRol()
        
        if (isMounted && response.success) {
          const data = response.tickets
          setAsistentes(data)
          
          // Extraemos las Unidades Académicas únicas para llenar el select del filtro
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 font-light text-xs uppercase tracking-widest">Cargando asistentes...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto p-4 md:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Selector de UA (Se llenará automáticamente basado en los resultados obtenidos) */}
      ...
      <GlassCard className="overflow-hidden" glowColor="purple">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          <span className="text-xs font-light text-slate-500 uppercase tracking-widest">Padrón de Asistentes</span>
