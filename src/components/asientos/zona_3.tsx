...
const NOGALES3_TOP_ROWS = [9,9,9,10,10,10,11,11,11,12,12,12,12,13,13,13,13];
const NOGALES3_BOTTOM_ROWS = [14,14,14,14,14,14,14,14,14,14,14,14,14,14,14];
...
export default function Zona3({onSeatSelect,occupiedSeats=[]}:SeatMapProps){
  ...
  const totalTop=NOGALES3_TOP_ROWS.reduce((a,c)=>a+c,0);
  const totalBot=NOGALES3_BOTTOM_ROWS.reduce((a,c)=>a+c,0);
          ...
          <div className="flex gap-1.5 justify-center">
            {Array.from({length:cnt}).map((_,si)=>{
              ...
              return <button key={id} onClick={()=>pick(id,info)} disabled={occ}
                className={"w-6 h-6 sm:w-7 sm:h-7 rounded-t-md text-[10px] font-bold transition-all duration-150 flex items-center justify-center border "+(occ?"bg-slate-300 border-slate-400 text-slate-500 cursor-not-allowed":isSel?"bg-rose-600 text-white border-rose-600 scale-110 shadow-md ring-2 ring-rose-500/30":"bg-white border-slate-300 text-slate-700 hover:border-rose-500 hover:bg-rose-50")}
                title={"Zona 3 (NOGALES) - Asiento "+id}>{num}</button>
            })}</div>
  ...
  return <div className="w-full flex flex-col items-center bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto">
    ...
    <button onClick={()=>setShow(s=>!s)} className={"px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all duration-150 border-2 shadow-sm mb-6 "+(show?"bg-rose-600 text-white border-rose-600 scale-105 shadow-md ring-2 ring-rose-500/30":"bg-white text-slate-700 border-slate-300 hover:border-rose-500 hover:bg-rose-50")}>{show?"Ocultar NOGALES":"Ver NOGALES"}</button>
    {show&&<>
      <div className="flex flex-col items-center gap-8 min-w-max">
        <div className="relative p-6 rounded-2xl bg-slate-200/50 border border-slate-300/60 flex flex-col items-center">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-rose-600 text-white text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full shadow-sm">NOGALES &mdash; Bloque Superior ({totalTop})</div>
          {renderBlock(NOGALES3_TOP_ROWS,0,"top")}</div>
        ...
        <div className="relative p-6 rounded-2xl bg-slate-200/50 border border-slate-300/60 flex flex-col items-center">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-rose-600 text-white text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full shadow-sm">NOGALES &mdash; Bloque Inferior ({totalBot})</div>
          {renderBlock(NOGALES3_BOTTOM_ROWS,NOGALES3_TOP_ROWS.length,"bottom")}</div>
      </div>
    </>}
    <div className="flex items-center gap-6 mt-8 text-xs font-medium text-slate-600">
      ...
      <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-rose-600"></div><span>Seleccionado</span></div>
