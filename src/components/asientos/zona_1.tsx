...
// Bloque 1 (Superior): 17 filas de 7 asientos cada una
const TOP_ROWS = Array(17).fill(7);

// Bloque 2 (Inferior): 15 filas de 10 asientos cada una
const BOTTOM_ROWS = Array(15).fill(10);
...
interface SeatMapProps {
  onSeatSelect?: (id: string, info: SeatSelectionInfo) => void;
...
export default function Zona1({ onSeatSelect, occupiedSeats = [] }: SeatMapProps) {
  ...
  const pick = (id: string, info: SeatSelectionInfo) => {
  ...
  const renderBlock = (rows: number[], start: number, name: string) => (
    <div className="flex flex-col items-center gap-2">
      {rows.map((cnt, ri) => {
        ...
        return (
          <div key={`${name}-${ri}`} className="flex items-center gap-1.5 justify-center">
            ...
            <div className="flex gap-1.5 justify-center">
              {Array.from({ length: cnt }).map((_, si) => {
                ...
                return (
                  <button
                    key={id}
                    onClick={() => pick(id, info)}
                    disabled={occ}
                    className={
                      "w-6 h-6 sm:w-7 sm:h-7 rounded-t-md text-[10px] font-bold transition-all duration-150 flex items-center justify-center border " +
                      (occ
                        ? "bg-slate-300 border-slate-400 text-slate-500 cursor-not-allowed"
                        : isSel
                        ? "bg-violet-600 text-white border-violet-600 scale-110 shadow-md ring-2 ring-violet-500/30"
                        : "bg-white border-slate-300 text-slate-700 hover:border-violet-500 hover:bg-violet-50")
                    }
                    title={`Zona 1 (TEH - TEQ - CUI) - Asiento ${id}`}
                  >
                    {num}
                  </button>
                );
            ...
            <span className="w-6 text-left font-mono text-xs font-bold text-slate-400 select-none ml-1">
              {row}
            </span>
          </div>
        );
  ...
  return (
    <div className="w-full flex flex-col items-center bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto">
      {/* Escenario */}
      <div className="w-64 sm:w-80 h-3 bg-slate-800 rounded-b-xl mb-6 flex items-center justify-center">
      ...
      <button
        onClick={() => setShow((s) => !s)}
        className={
          "px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all duration-150 border-2 shadow-sm mb-6 " +
          (show
            ? "bg-violet-600 text-white border-violet-600 scale-105 shadow-md ring-2 ring-violet-500/30"
            : "bg-white text-slate-700 border-slate-300 hover:border-violet-500 hover:bg-violet-50")
        }
      >
        {show ? "Ocultar TEH - TEQ - CUI" : "Ver TEH - TEQ - CUI"}
      </button>
      ...
      {show && (
        <div className="flex flex-col items-center gap-8 min-w-max">
          ...
          <div className="relative p-6 rounded-2xl bg-slate-200/50 border border-slate-300/60 flex flex-col items-center">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-violet-600 text-white text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full shadow-sm">
              TEH - TEQ - CUI &mdash; Bloque Superior ({totalTop})
            </div>
            {renderBlock(TOP_ROWS, 0, "top")}
          </div>
          ...
          <div className="relative p-6 rounded-2xl bg-slate-200/50 border border-slate-300/60 flex flex-col items-center">
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-violet-600 text-white text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full shadow-sm">
              TEH - TEQ - CUI &mdash; Bloque Inferior ({totalBot})
            </div>
            {renderBlock(BOTTOM_ROWS, TOP_ROWS.length, "bottom")}
          </div>
        </div>
      )}
