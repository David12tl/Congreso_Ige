...
const TOP_ROWS = [9,9,9,10,10,10,11,11,11,12,12,12,12,13,13,13,13];
const BOTTOM_ROWS = [14,14,14,14,14,14,14,14,14,14,14,14,14,14,14];
...
export default function Zona1({onSeatSelect,occupiedSeats=[]}:SeatMapProps){
  ...
  return <div className="w-full flex flex-col items-center bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto">
    ...
    <button onClick={()=>setShow(s=>!s)} className={"px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-wider transition-all duration-150 border-2 shadow-sm mb-6 "+(show?"text-white scale-105 shadow-md ring-2":"bg-white text-slate-700 border-slate-300 hover:bg-slate-100")} style={show?{backgroundColor:ACCENT,borderColor:ACCENT,boxShadow:"0 0 0 3px "+ACCENT+"40"}:{}}>{show?"Ocultar TEH - TEQ - CUI":"Ver TEH - TEQ - CUI"}</button>
    {show&&<>
      <div className="flex flex-col items-center gap-8 min-w-max">
        <div className="relative p-6 rounded-2xl bg-slate-200/50 border border-slate-300/60 flex flex-col items-center">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-white text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full shadow-sm" style={{backgroundColor:ACCENT}}>TEH - TEQ - CUI &mdash; Bloque Superior ({totalTop})</div>
        ...
        <div className="relative p-6 rounded-2xl bg-slate-200/50 border border-slate-300/60 flex flex-col items-center">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-white text-[10px] font-black uppercase tracking-wider px-3 py-0.5 rounded-full shadow-sm" style={{backgroundColor:ACCENT}}>TEH - TEQ - CUI &mdash; Bloque Inferior ({totalBot})</div>
